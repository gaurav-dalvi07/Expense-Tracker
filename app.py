...
def edit(id):
    ...
    if request.method == "POST":
        ...
        cur.execute(
            "UPDATE expenses SET amount=?, category=?, date=? WHERE id=? AND user=?",
...
def set_budget():
    ...
    cur.execute("SELECT id FROM budgets WHERE user=? AND month=?", (user, month))
    existing = cur.fetchone()

    if existing:
        cur.execute("UPDATE budgets SET amount=? WHERE user=? AND month=?", (amount, user, month))
    else:
        cur.execute("INSERT INTO budgets (user, month, amount) VALUES (?,?,?)", (user, month, amount))
...
def clear_month():
    ...
    cur = conn.cursor()
    cur.execute(
        "DELETE FROM expenses WHERE user=? AND strftime('%Y-%m',date)=?",
        (session["user"], data["month"])
    )
    conn.commit()
    conn.close()
    return jsonify({"status":"ok"})
